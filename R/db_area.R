...
db_area <- function(data, order, cat1, cat2, dpi = 320, seed = 42,
  ...
  ## them first and them use this package
  ...
  highlight <- data %>%
    dplyr::mutate(cat1 = round(cat1, digits = 1),
                  cat1 = ifelse(dplyr::row_number(order) == 1L | dplyr::row_number(order) == dplyr::n(),
                                paste0(cat1,"%"),
                                cat1)) %>%
    dplyr::pull(cat1)

  ## Gets the first and last itens in the set of order and converts them to numbers
  ord1 <- data %>%
    dplyr::slice(1L) %>%
  ...
  lft_area <- tibble::tibble(
    ...
    dplyr::mutate(x = z + 2*mean(data$cat1)/3 + mean(data$cat2),
                  x = ifelse(x > 100, 100, x))

  ## Defines the names of the categories
  categ_names <- data %>%
    dplyr::slice(1L:2L) %>%
    dplyr::summarise(x1 = mean(cat1)/2,
  ...
  ggplot2::ggsave(file, plot = p, dpi = dpi,
                  width = 22, height = 28)
